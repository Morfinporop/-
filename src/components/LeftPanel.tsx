        ...
        <div className="flex flex-col h-full bg-white border-r border-gray-200">
          <div className="flex items-center justify-between pt-4 pb-3 px-4 border-b border-gray-200">
            <div className="flex items-center gap-3 min-w-0">
              <button onClick={onAvatarClick} className="w-10 h-10 rounded-full overflow-hidden flex-shrink-0 flex items-center justify-center border border-gray-200 bg-gray-100">
              ...
              <span className="text-sm font-semibold text-gray-800 truncate">{user?.name || 'Гость'}</span>
            </div>
            <button onClick={onPanelToggle} className="w-9 h-9 rounded-full bg-white border border-gray-200 flex items-center justify-center hover:bg-gray-100 transition-colors">
          ...
          <div className="px-4 py-3 flex items-center gap-2">
            <div className="flex-1"><input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Поиск чатов..." className="w-full bg-transparent outline-none text-xs pb-1 text-gray-800 placeholder:text-gray-500" /><div className="h-px mt-1 bg-gray-300" /></div>
            <button onClick={onNewChat} className="w-9 h-9 rounded-full bg-white border border-gray-200 flex items-center justify-center hover:bg-gray-100 transition-colors">
          ...
          <div className="px-4 pb-2"><span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider px-1">Недавнее</span></div>
          <div className="flex-1 overflow-y-auto px-4 pb-4">
            {sorted.map(c => (
              <div key={c.id} className="relative mb-0.5" ref={menuId === c.id ? ref : undefined}>
                <div onClick={() => onSelectChat(c.id)} className="flex items-center justify-between px-3 py-2 rounded-xl cursor-pointer transition-colors text-sm max-w-[240px] mx-auto hover:bg-gray-100" style={{ background: c.id === activeChatId ? 'rgba(0,0,0,0.04)' : '', borderLeft: c.id === activeChatId ? '3px solid #1a1a1a' : '3px solid transparent' }}>
                  <span className="truncate flex-1 text-gray-600 text-xs font-medium pr-1">{c.title}</span>
                  <button onClick={e => { e.stopPropagation(); setMenuId(menuId === c.id ? null : c.id); }} className="p-1 rounded-lg hover:bg-gray-200 flex-shrink-0"><svg width="12" height="12" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="5" r="1.5" fill="#aaa"/><circle cx="12" cy="12" r="1.5" fill="#aaa"/><circle cx="12" cy="19" r="1.5" fill="#aaa"/></svg></button>
                </div>
                {menuId === c.id && <div className="absolute right-2 top-10 z-50 rounded-xl shadow-md py-1 bg-white border border-gray-200 min-w-[100px]"><button onClick={() => { onDeleteChat(c.id); setMenuId(null); }} className="w-full text-left px-3 py-2 text-xs text-red-500 hover:bg-red-50 rounded-lg transition-colors">Удалить</button></div>}
              </div>
            ))}
          </div>
        </div>
      ) : (
        <div className="flex flex-col items-center justify-start pt-6 h-full bg-white border-r border-gray-200">
          <div className="flex flex-col items-center gap-3 p-3">
            <button onClick={onAvatarClick} className="w-14 h-14 rounded-full overflow-hidden flex items-center justify-center border-2 border-gray-200 bg-gray-100">
