  ...
  return (
    <Dialog.Root open onOpenChange={v => { if (!v) onClose(); }}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-sm mx-4 rounded-3xl p-7 bg-surface border border-border shadow-lg animate-fade-in outline-none">
          <Dialog.Close className="absolute top-4 right-4 w-8 h-8 rounded-full flex items-center justify-center hover:bg-surface-dim transition-colors btn btn-light">
          ...
          <Dialog.Title className="text-lg font-semibold text-text mb-7 text-center">{mode === 'login' ? 'Вход в аккаунт' : 'Регистрация'}</Dialog.Title>
          <div className="flex flex-col gap-6">
            {mode === 'register' && (
              <div>
                <input type="text" value={name} onChange={e => setName(e.target.value)} onKeyDown={kd} placeholder="Введите имя" className="w-full bg-transparent outline-none text-sm text-text pb-1 placeholder:text-text-muted" />
                <div className="h-px mt-1 bg-border-strong" />
              </div>
            )}
            <div>
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={kd} placeholder="Введите почту" className="w-full bg-transparent outline-none text-sm text-text pb-1 placeholder:text-text-muted" />
              <div className="h-px mt-1 bg-border-strong" />
            </div>
            <div>
              <input type="password" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={kd} placeholder="Введите пароль" className="w-full bg-transparent outline-none text-sm text-text pb-1 placeholder:text-text-muted" />
              {mode === 'register' ? (
          ...
          <div className="flex justify-center mt-6">
            <button className="w-10 h-10 rounded-full flex items-center justify-center border border-border hover:bg-surface-dim btn btn-light">
          ...
          <button onClick={submit} disabled={loading} className="w-full mt-5 py-3 rounded-2xl text-sm font-semibold text-primary-fg bg-accent disabled:opacity-50 btn btn-dark">
            {loading ? '...' : mode === 'login' ? 'Войти' : 'Зарегистрироваться'}
          </button>
          <div className="text-center mt-4">
            <button onClick={() => { onModeChange(mode === 'login' ? 'register' : 'login'); setError(''); }} className="text-xs text-text-muted hover:text-text transition-colors">
